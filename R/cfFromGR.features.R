...
cfFromGR.features <- function(query, subject, minPropOverlap=0.5, verbose=FALSE)  {
    ...
    stopifnot(any(!is.na(seqlengths(subject))))  # check that the subject has seqlengths for all chroms. Required to calculate gaps. 
...
# TODO add to GL
# Specify unit-tests (make GRanges with seqinfo modelled on SacCer3 but not using SacCer3)
# documentation. 
