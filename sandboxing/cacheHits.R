...
# SacCer TEST RUN WITH LAYER MODS ----------
...
while(i <= n.runs) {
  ...
  if( i %% 100 == 0)  {   # export tables every 100th iteration
    write.table(statsTrace, file=paste0("simpleSpread.", n.runs, ".", use.cache, ".statsTrace.tab"), sep="\t", quote=F, row.names=F)
    write.table(coverTrace, file=paste0("simpleSpread.", n.runs, ".", use.cache, ".coverTrace.tab"), sep="\t", quote=F, row.names=F)
  }
  i <- i +1
}
)
