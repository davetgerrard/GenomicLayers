...
# THIS VERSION:  5 layers, 400bp around the tss
# 
...
source('/mnt/fls01-home01/mqbssdgb/scratch/seqPredict/predictfromsequence/scripts/pfs.scoreMeasures.R')
...
logCycle<- 1 
maxNoChange<- 100
runName <- "pfs_layer5_chr22_400bp_ppv"
...
test_function <- function(layerList, targetLayer, target.vec)  {
  score <- score.hits(layerList$layerSet[[targetLayer]], target.vec, method=ppv)
  return(score)
}
