...
GENOME <- 'mm9'
par.outFile <- paste0(OUTPUTDIR, "parallel.runTest.16.txt")
...
# instead of waves, make one large XFS. 
...
system.time(
  resultsVec <- clusterApplyLB(cl, 1:16, fun=function(x) layerBindAndScore(layerList=layerList.X, factorSet = XFS.10, iterations = n.iters*n.waves,
