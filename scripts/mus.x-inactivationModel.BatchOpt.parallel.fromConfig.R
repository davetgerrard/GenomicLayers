...
# THIS VERSION: implements 16 core parallel duplicated runs at each stage, keep only if MIN score exceeds current best score.
...
print("set optimisation initial values")
n.cores <- 16
new.XFS <- XFS
keep.XFS <- XFS
...
for(j in 2:n.optCycles) {
  ...
  if(j %% saveEvery == 0)  {
    ...
    save(keep.XFS, optim.scores.vec, file=outFile)
