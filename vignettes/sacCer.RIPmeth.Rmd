...
Create a named link to the genome, inspect it and remind ourselves of the degenerate IUPAC codes.

```{r checkGenome, echo=TRUE}
genome <- BSgenome.Scerevisiae.UCSC.sacCer3

seqinfo(genome)
IUPAC_CODE_MAP   # check the IUPAC codes.
```
...
sacCer.LS <- createLayerSet.BSgenome(BSgenome.Scerevisiae.UCSC.sacCer3,
                                     n.layer=2, 
...
The first binding factor will bind to regions of the genome of 400bp that have an AT% greater than or equal to 70%. Where it binds, it will set the H3K9me3 layer to state 1 (rather than 0 or empty).
...
It could be further extended to incorporate spreading of the H3K9me3 mark by HP1.  At time of writing, the BSgenome is immutable, so we cannot "mutate" the methylated Cytosines. 
